...
├── gbrain.yml
├── .fulltext/                              # 15 部完整全文阅读副本；点目录，GBrain 与公开导出均排除
├── metadata/
│   ├── longform_split_registry.json        # 15 部可逆切章注册表
│   └── public_assets_manifest.json         # 公开资产权利清单（README 肖像等）
├── caute_ru_markdown/
...
```text
longform_split=0 longform_verified=15 longform_materialized=0 registered=15
...
### 7.1 完整全文阅读副本（`.fulltext/`）

2026-06-12 起，15 部作品的完整单文件重建版镜像各自原路径存于点目录 `.fulltext/`，便于通读整篇文章。每个文件由 `scripts/split_longform_markdown.py --materialize` 从章节无损重建，与对应 `source/<work>.md.snapshot` 逐字相同（SHA-256 等于 manifest `original_sha256`）。

设计要点：

- GBrain 的文件走查跳过 `.` 开头目录，故 `.fulltext/` 不被索引——dry-run 实测仍为 658 个文件，无 `.fulltext` 条目，页数零增长。
- `scripts/prepare_gbrain_markdown.py` 与 `scripts/export_public.py` 的 `SKIP_PARTS` 均含 `.fulltext`，公开导出文件集合不变；导出 verify 另设 `.fulltext/` 前缀禁止项做双保险。
- 这些是阅读副本而非权威来源；权威可逆存储仍是章节文件与快照。内容为原文语料，`redistribution_approved: "false"`，不公开再分发。
- 章节变更后须重跑 `--materialize`；发布流水线的 `split_longform_markdown.py --check` 已校验全文与章节同步（缺失或过期即失败）。
...
python3 scripts/split_longform_markdown.py --materialize   # 章节变更后重建 .fulltext/ 全文
...
覆盖新增：全文重建逐字一致、`.fulltext` 不进公开导出、章节变更后全文过期可被 `--check` 检出。
